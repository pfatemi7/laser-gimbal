...
- **Visual servoing** to keep hand centered using proportional control
- **External camera support** with auto-detection and manual configuration
...
- **Visual feedback** with hand tracking overlay and error visualization
...
#### Visual Servo Tracker (Recommended)
```bash
cd laptop
python visual_servo_tracker.py
```

#### Quick Launcher
```bash
python run_visual_servo.py
```
...
#### Visual Servo Mode (Default)
- Position your hand in front of the camera
- The system will try to keep your hand centered in the frame
- **Visual servoing**: Uses proportional control to maintain center position
- **Error visualization**: Red arrow shows error direction and magnitude
- **Target angles**: Displayed in real-time as the system adjusts

#### Legacy Direct Mapping Mode
- Position your hand in front of the camera
- The servos will follow your hand movement directly
- **Pan control**: Left edge of frame → ~170° servo position, Right edge → ~10°
- **Tilt control**: Top of frame → 0°, Bottom of frame → 90°

#### Camera Configuration
```bash
# Auto-detect external camera (default)
python visual_servo_tracker.py

# Use specific camera index
python visual_servo_tracker.py --camera-index 1

# Use specific camera path
python visual_servo_tracker.py --camera-path /dev/video1

# Adjust control parameters
python visual_servo_tracker.py --kp 0.3 --ki 0.1 --enable-integral
```
...
The new visual servo tracker implements a closed-loop control system that:
1. **Detects hand position** in normalized coordinates (0-1)
2. **Computes error** from center position:
   - `ex = 0.5 - hand_x` (positive if hand is left of center)
   - `ey = hand_y - 0.5` (positive if hand is below center)
3. **Updates target angles** using proportional control:
   - `pan_target += kp * ex`
   - `tilt_target += kp * ey`
4. **Maintains angle limits**: Pan (10-170°), Tilt (0-90°)

### Control Parameters
- **`--kp`**: Proportional gain (default: 0.5) - higher = more responsive
- **`--ki`**: Integral gain (default: 0.0) - helps eliminate steady-state error
- **`--enable-integral`**: Enable integral control (disabled by default)

### Camera Configuration
- **Auto-detection**: Prefers `/dev/video1` (external), falls back to index 0
- **Manual index**: `--camera-index 1` for specific camera
- **Manual path**: `--camera-path /dev/video1` for specific device
...
├── laptop/
│   ├── visual_servo_tracker.py     # NEW: Visual servo tracker
│   ├── run_visual_servo.py         # NEW: Quick launcher
│   ├── hand_tracker.py             # Legacy dual-axis hand tracker
